...
router.get("/dashboard-data", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // Update starting weight if needed
    ...
    const currentWeight =
      weightDataKg.length > 0
        ? weightDataKg[weightDataKg.length - 1].weightKg
        : startingWeight;
    const progress =
      startingWeight && currentWeight ? currentWeight - startingWeight : 0;

    const dailyMacro = await DailyMacro.findOne({ userId, date: today });
    const recentMeal =
      dailyMacro && dailyMacro.meals.length > 0
        ? dailyMacro.meals[dailyMacro.meals.length - 1]
        : null;
    ...
    const completedExercises = routines.reduce(
      (acc, routine) =>
        acc + routine.exercises.filter((ex) => ex.completion === "Yes").length,
      0
    );
    ...
    res.json({
      ...
      weight: {
        ...
        progressMessage:
          startingWeight && currentWeight
            ? progress < 0
              ? `You have lost ${Math.abs(progress).toFixed(2)} kg so far! 😊`
              : `You have gained ${progress.toFixed(2)} kg so far! 😊`
            : "",
      },
      meal: {
      ...
      calories: {
        ...
        message: dailyMacro
          ? caloriesConsumed > calorieGoal * 1.2
            ? "You have exceeded your caloric intake!"
            : "You have reached your caloric goal!"
          : "",
      },
      exercises: {
