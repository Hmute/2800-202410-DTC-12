document.addEventListener("DOMContentLoaded", function () {
  ...
  // Update the UI with weight entries
  function updateUI() {
    ...
    data.forEach((entry) => {
      ...
      entryElement.innerHTML = `
                ...
                <span class="badge ${
                  entry.change > 0 ? "badge-danger" : "badge-success"
                } badge-pill">
  ...
  // Handle form submission to add new weight entry
  addWeightForm.addEventListener("submit", async (event) => {
    ...
    const date = new Date(
      ...
      currentDate.getDate()
    ).toISOString(); // Normalize to start of the day
    ...
    if (response.ok) {
      const updatedEntryKg = {
        ...newEntry,
        weightKg: parseFloat(newEntry.weightKg).toFixed(2),
      };
      ...
      const previousEntry = weightData.find(
        (entry) => new Date(entry.date).setHours(0, 0, 0, 0) < normalizedDate
      );
      const change = previousEntry
        ? (
            ((weightKg - previousEntry.weightKg) / previousEntry.weightKg) *
            100
          ).toFixed(2)
        : 0;
      ...
      const kgIndex = weightData.findIndex(
        (entry) => new Date(entry.date).setHours(0, 0, 0, 0) === normalizedDate
      );
      if (kgIndex !== -1) {
      ...
      const progressValue = (currentWeightValue - startingWeightValue).toFixed(
        2
      );
